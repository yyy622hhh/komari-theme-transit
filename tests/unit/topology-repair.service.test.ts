...
function createDeps(overrides: Partial<TopologyRepairDeps> & { manager: TopologyRepairManagerLike }): TopologyRepairDeps {
  return {
    ...
    isEntryTaskSessionOwned: () => false,
    rememberEntryTaskCreated: () => {},
    ...overrides,
  }
}
...
describe('runTopologyProbeRepair node identity recovery', () => {
  ...
  test('recovers a route whose relay was renamed in Komari, using the cached uuid, and rewrites the stored name', async () => {
    // 模拟此前主题已经见过这台线路机叫「Relay-JP-old」，把 uuid 记进本地缓存。
    recordTopologyNodeIdentity([{ uuid: relay.uuid, name: 'Relay-JP-old' }])

    const renamedRelay: NodeData = { ...relay, name: 'Relay-Tokyo' }
    const staleRoute = route({ sourceName: 'Relay-JP-old', taskFilter: 'Transit-Relay-JP-old-to-Exit-SG' })
    const { manager, log } = createManager([staleRoute])

    const outcome = await runTopologyProbeRepair(createDeps({
      nodes: () => [renamedRelay, landing],
      manager,
      planWorkingHopTask: async source => hopPlan({
        needsCreation: false,
        task: { ...hopPlan().task, name: `Transit-${source.name}-to-Exit-SG` },
      }),
    }))

    expect(outcome).toBe('repaired')
    expect(log.saveCalls).toEqual([{ lockHeld: true }])
    expect(staleRoute.nodes[1]?.name).toBe('Relay-Tokyo')
    expect(staleRoute.metrics[1]?.nodeName).toBe('Relay-Tokyo')
    expect(staleRoute.metrics[1]?.taskFilter).toBe('Transit-Relay-Tokyo-to-Exit-SG')
  })

  test('leaves a route untouched when the name has no match and no identity was ever cached', async () => {
    const staleRoute = route({ sourceName: 'Ghost-Relay' })
    const { manager, log } = createManager([staleRoute])
    const outcome = await runTopologyProbeRepair(createDeps({ manager }))
    expect(outcome).toBe('no-op')
    expect(log.saveCalls).toEqual([])
    expect(staleRoute.nodes[1]?.name).toBe('Ghost-Relay')
  })
})
...
describe('runTopologyProbeRepair entry segment', () => {
  ...
  test('escalates a dead entry task the current session created, creating the replacement before cleaning up the old one', async () => {
    ...
    const outcome = await runTopologyProbeRepair(createDeps({
      manager,
      isEntryTaskSessionOwned: taskId => taskId === 55,
      planEntryProbeTask: async () => entryPlan({
        needsCreation: true,
        probe: { type: 'tcp', port: 443 },
