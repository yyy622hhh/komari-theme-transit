...
- 重做网络总览：使用不透明数据卡片、三网独立列和精简回程徽章；资源、流量及账单信息分层展示，保留异常原因和采样详情，改善深浅色文字对比度。
- 简化线路拓扑：连接关系、当前探测状态和近 1 小时均值优先展示，历史评分、基线、排名和事件收纳在线路详情。箭头仅表示已匹配来源的探测方向，不暗示用户访问路径。
- 首页增加总览／拓扑页内导航，资源汇总默认折叠；手机纵向显示拓扑，长节点名称完整换行。更新主题预览和文档截图。

### Fixed

- 固定部分回程结果的运营商列位，避免缺少联通记录时将电信或移动结果错放到相邻列。
- 无线路的访客展示只读空状态，拓扑导航不再指向不存在的区域；补齐页面底部导航高亮及动画开关下的详情往返回归。

### Compatibility / Testing

- 不修改现有监测目标、任务历史、回程规则、权限或伴生协议；保留默认 compact 密度及普通卡片模式。伴生插件及助手只同步发布版本元数据，旧助手可继续使用，无需批量升级。
- 新增跨浏览器导航、窄屏布局、部分回程列位和访客权限回归；视觉基线由固定 macOS CI 生成并审查，不放宽像素断言。

## [1.4.1] - 2026-08-31

### Fixed

- 修复已取消请求继续启动、卸载重挂期间旧请求回写和公共任务列表失效后被旧请求回填的问题；创建/复用响应丢失但确认落地时同步刷新缓存。同名任务合并统计与样本两类成功证据，不因统计暂缺选回旧 ID。
- 指标批次对异常响应保证收尾并回退；同步目标故障在完整逻辑批次上统一判断，跨 50 台请求边界不改变 60% 分母，部分批次缺失时不推断公共故障。
- 三网更新时间排除未启用和无样本地区，以有效样本中的最早时间展示整体延迟；P99/P50 正确显示为无量纲波动倍率，不再标为毫秒抖动，告警算法保持不变。
- 助手花名册新增可选执行租约到期时间，正在采集的助手不再因 40 秒未轮询误报未安装；租约到期、完成后更新状态，旧插件缺字段继续兼容。未领取任务直至 TTL 到期归为助手未连接，而非探测失败。
- 配置导入更换文件时立即撤销旧预览，取消、切换回滚或退出面板后丢弃迟到的读取；读取失败不再沿用旧文件。导入与回滚共享写入互斥，阻止重复确认和交叉提交。
- 三网“全部地区”的平均延迟与抖动按成功样本加权，不把无响应当作零延迟；可用率与加权失败率保持一致，历史采样格按时间对齐，不再将不同地区的旧异常拼到最新时间。
- 首页三网任务复用广州/广东预设别名，保留精确匹配与同名新旧 ID 隔离；缓存同步别名并更新格式，异步写入缓存索引遇到配额/存储失败时安全降级。

### Security

- 手动采集写回使用 Python 隔离模式启动，阻止当前工作目录、用户 site-packages 和 `PYTHON*` 环境变量注入模块。新增实际脚本/真实解释器启动回归及无隔离的阳性对照，不以 HTTP 模拟测试代替启动边界验证。
- 手动采集脚本的 `--push` 强制 HTTPS 与证书校验、禁止自动重定向；管理员密钥改用 `--key-file` 或终端隐藏输入，拒绝明文 `--key` 和 `KOMARI_API_KEY`。密钥文件验证实际打开文件的属主、普通文件类型及 `0400/0600` 权限，禁止符号链接；响应错误与成功日志不回显服务端原文。旧 cron 必须迁移，已暴露的旧密钥需管理员轮换；不影响伴生助手的只采集模式。

### Reliability / Safety

- 图表 HTML 提示统一转义节点上报的设备名、任务名与序列标签，并限制动态色值。助手安装入口改用私有下载目录，任一下载失败即中止；手动运行也使用独占私有目录和禁止覆盖的 `0600` 请求文件。
- 新助手仅通过 HTTPS JSON 请求体发送节点凭据，拒绝远端 HTTP 和命令行 `--token`，不回退 URL token。先升级伴生插件，再逐台升级助手；新插件仍兼容旧助手，旧助手的 URL 日志风险需随节点升级消除。
- 伴生插件对轮询和结果按认证节点限流，并返回 `Retry-After`；已完成结果的重复提交不再反复写盘和输出接受日志。补充 XSS、凭据传输、临时文件、安装失败及重放行为回归。
- 拓扑清理与补偿删除必须匹配创建时完整快照；被管理员改作他用或仅有旧 ID 的任务保留并记录核对提示。助手安装使用受限临时文件原子替换配置，任何关键步骤失败均中止；升级后重启并核验服务。
- 插件读取故障恢复时按节点/城市收敛重复活跃任务，优先保留有效租约，冗余任务按失败收口且不破坏其他租约索引。助手结果等待覆盖十分钟任务 TTL 与传输余量，不再在合法结果回传前按旧版 150 秒时限提前退出；固定 `admin:exec` 等待不变。
- 删除旧监测任务前同时复查原任务与替代任务完整快照；替代任务被修改或删除时中止并保留原任务。插件恢复读盘时按已上报字段合并助手状态，普通心跳不清空探测历史；拓扑回退值明确标为备用基线，不再冒充近 1 小时均值。
- 补齐三网卡片的失败率列说明与拓扑详情口径：历史失败不再称为当前异常或实时稳定，TCP 失败不称业务丢包；新增当前恢复/持续失败/过期/证据不足但历史仍为 3.3% 的浏览器回归。
- 监测目标操作使用应用级状态、异常收尾和 Web Locks 同源互斥；创建替代任务与删除旧任务前重验权限和原快照，关闭弹窗不取消，刷新仅回查本地记录，不自动继续写入。删除结果不明时保留已验证的新任务，补偿失败列出残留 ID。
- 候选门槛只使用去重、排序、创建时间之后的原始样本；单节点要求最近连续五次成功，迁移前重新核对候选、快照、有效期和最新证据。来源离线与采样不足单独报错，保留原任务离线节点的分配。
- 修复助手 Bash 抖动变量初始化与旧 Bash 的中文变量边界问题，新增无需 root、无需真实等待的 HTTP/退避执行测试。

### Changed

- 按确认稿调整三网卡片与拓扑：当前状态独立着色，历史均值/失败率使用两层表头，采样格独占整行；采样详情按需展开并显示任务、更新时间与最近成功。拓扑当前状态置于采样线上方，历史统计置于下方，不用绝对定位挤压文本。补充 320–1440px 深浅色文字边界回归。
- 健康中心、首页三网及拓扑将“当前状态”与“近 1 小时统计/评分”分开，恢复不清空历史，TCP 使用探测失败率。最近三个精确样本决定状态，过期与离线单独显示；Komari 精确样本不可用时显示未知，不使用旧记录接口的汇总桶推断恢复。
- 插件 `/health` 新增可选存储状态；写盘故障仍提供探测，保留内存及待保存状态，后续请求按 15/30/60 秒退避重试。设置向导、诊断及脱敏报告提示重启风险，错误只暴露分类；旧插件显示“未报告”。

### Testing / Compatibility

- 新增实际文件读写故障、损坏恢复、操作锁、刷新回查、原始样本与助手 Shell 行为测试；真实 Komari 实验室增加插件安装、租约、升级、重启和 v1.4.0 状态读取，保留旧版本固定 `admin:exec` 路径测试。
- 不增加 IPv6、入站能力或系统权限，不自动迁移真实目标，不修改回程判定和标签格式；旧助手与 `state-v1.json` 无需迁移。发布与生产部署仍需明确授权和完整 CI 门禁。

## [1.4.0] - 2026-08-26

### Added

- 新增管理员“监测目标健康”中心：汇总九个地区/运营商任务最近一小时的成功率、异常节点和公共目标同步故障，支持通过最多五台在线节点验证内置 TCP/53 或自定义 ICMP/TCP 候选目标。
- 新增确认式安全迁移与“重建当前任务”：使用新 Ping 任务 ID 等待首个成功样本后再清理旧任务，从而隔离旧目标的失败历史；超过 30 分钟的 Transit 临时验证任务会在再次打开健康中心时清理。
- 回程伴生插件花名册新增助手版本、最后任务、最后成功、最近错误和耗时；插件通过 Komari `__storageDir__` 原子持久化未过期任务、批次、租约与 30 天助手状态。

### Changed

- 节点助手正常空轮询改为 12–18 秒抖动；网络错误和 5xx 使用带 ±20% 抖动的 15→30→60→120→300 秒退避，认证/插件缺失错误固定五分钟并抑制重复日志，合法数字 `Retry-After` 优先。
- 地区三网统计使用忽略分隔符后的精确任务名；迁移期同名任务优先选择已有成功样本的任务，其次选择更新 ID，避免新旧历史混算。

### Compatibility

- 不自动修改现有 Ping 任务，不增加 IPv6 探测，不改变 `transit-route:` 标签或旧 Komari 的 `admin:exec` 回退路径；v1.3.12 及更旧助手可继续轮询，仅在设置向导显示版本提示。
...
- 提升暗色模式节点卡中在线时长、资源明细、网络概览和三网回程等次要文字的对比度；浅色模式、主信息和告警色保持不变。
...
- 节点负载、详情页流量峰值和 Ping 历史的去重请求改为按消费者独立取消；切换节点、关闭组件或释放最后一个共享订阅时，不再误取消仍被其他图表使用的同一底层请求。
- 修复共享历史请求取消尚未结束时立即重新订阅的竞态，新消费者会等待旧请求收尾并重新发起，不会继承已经中止的 Promise。
- 应用初始化模块只缓存成功结果；部署切换或网络波动造成动态模块首次加载失败后，“重试”现在会真正重新加载，不再重复返回永久失败的缓存 Promise。

### Testing

- 新增负载与 Ping 去重请求的消费者取消隔离、初始化同步/异步失败重试、并发去重和成功缓存回归测试；完整单元、浏览器功能、视觉、性能、构建、发布结构和依赖安全门禁通过。

## [1.3.8] - 2026-08-24

### Fixed

- 桌面端拓扑线路重新占满内容区，消除落地机右侧的大块空白；窄屏和四节点线路仍可横向滚动，不挤压节点名称和逐段指标。
- 静态和等待自动规划的拓扑段不再绘制容易被误认为历史数据的占位采样点，改为明确显示“静态”或“待探测”。
- README 更新到当前三态探测模型，并重新生成暗色、亮色、拓扑管理器和发布包预览图。
