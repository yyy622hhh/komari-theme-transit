...
- 拓扑管理及后台修复保留明确绑定的探测协议、端口和目标，不再把 TCP 强制迁回 ICMP；持续失败时保留原任务及真实失败数据，等待管理员处理。来源或落地地址已变化的无效绑定仍重新规划。
- 入口任务同样保留显式绑定，不因历史失败自动切换目标；管理提示按实际使用的探测方式说明问题，不再声称未执行的协议也已失败。

### Changed

- 健康中心改为推荐待验证的 TCP 候选，更新北京三网内置候选；相同目标不重复创建验证任务。候选不代表实时可用性，不自动迁移线上任务，上海、广州及现有 ICMP 配置继续兼容。
- 统计口径随任务协议显示：ICMP 为丢包率，TCP 为连接失败率；本次不修改历史统计公式或回程判定。

### Testing

- 补充健康/全失败 TCP 绑定保留、错误落地地址修复、候选端口映射、重复候选和浏览器迁移成功/补偿回归。
