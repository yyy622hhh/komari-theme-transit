...
- **原生服务器列表**：登录后可在主题内查看实时服务器清单、筛选状态并进入详情或节点运维；配置管理仍沿用 Komari 自带后台。
...
- **服务器列表**：按名称、地区、IP、系统和 CPU 搜索，筛选在线、离线或维护节点，并查看实时资源和流量。
...
Transit 不再内嵌或再分发 `komari-web` 构建物。登录后的服务器列表由 Transit 自有代码展示主题已经接收的实时节点数据，并提供详情和维护入口；新增/删除 Agent、密钥、Ping 任务、通知、主题、插件、数据库、终端、账号和权限等配置能力继续使用 Komari 安装包自带的官方后台。主题顶部和服务器列表中的后台入口都直接进入官方地址。
