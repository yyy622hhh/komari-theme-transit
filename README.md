...
| 回程线路 | 自动检测三网回程走哪条骨干网（CN2GIA/163/9929/CMIN2 等），无需逐台安装或配置         |
...
节点详情页会显示三条线路徽章；首页的普通节点卡使用紧凑徽章，Transit 运维节点卡则使用独立的「三网回程」面板，与网络概览并排、三网质量位于下一行。线路按档次着色（精品/优质/普通），数据方向按「节点 → 骨干线路 → 国内运营商」排列。鼠标悬停、键盘聚焦或触屏点击都可以查看逐跳 ASN、判定依据和采集时间；点击路线本身不会误进入节点详情。没有采集数据时整块不渲染。
...
**不需要你逐台去装。** 主题通过 Komari 自带的远程执行（`admin:exec`）把命令一次下发给所有该测的节点，结果回来后自动判定并写回标签。零 SSH、零 cron、零 API Key。

默认自动进行：已登录管理员打开首页约 20 秒后，主题会挑出「在线，且没测过或结果已超过 7 天」的节点跑一轮。回程几周才变一次，这个条件天然把频率压到每台约每周一次——没有间隔设置要调。首页工具栏上还有一个「检测回程 N」按钮，刚加完机器不想等的时候可以手动点，N 是当前待测台数；没有待测节点时按钮不出现。
...
- 节点没装 `traceroute` 时会明确回报「节点未安装 traceroute」，不会去动那台机器上的软件包，也不会因为一台失败而影响其余节点。失败过的节点在本次会话里不再自动重试，避免在跑不通的机器上反复执行命令；装好之后点一次手动按钮即可重来。
- 后台标签页不会自动采集，只有当前可见的页面才会。
- 三家运营商一个骨干跳都没认出来时判为采集失败，**不写回**——否则一次网络抖动就会把上次的好结果覆盖成「未见骨干」，还让这台机器七天内不再重测。
- 管理员账号启用了 2FA 时，`admin:exec` 属敏感操作会被拒绝。这种情况下主题会停止自动重试并在按钮上说明，改用下面的节点侧脚本即可。
- 需要 `traceroute`（ICMP 模式需 root，Komari agent 通常已是 root）。

如果你的场景不适合让主题执行命令（比如管理员账号开了 2FA、或者策略上禁止），仓库里另有 [`scripts/collect-return-route.sh`](scripts/collect-return-route.sh)，可以在节点上挂 cron 自己采集写回，格式完全一致：
