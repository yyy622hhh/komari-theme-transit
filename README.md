...
### v1.0.29 - v1.0.37 拓扑数据一致性更新

- 多条线路的节点组和指标组严格按原始位置对齐，空指标不再导致后续线路借用前一条的延迟。
- 支持“入口 → 目标”两节点线路，只渲染和保存实际存在的一段链路。
- Ping 任务按来源 UUID、完整名称和目标 IP 精确匹配；更换线路机或落地机时会自动重新规划，不会套用旧节点任务。
- 过期 Ping 缓存不再标记为“实时稳定”；无新数据时明确使用备用基线。
- 节点元数据刷新失败不会丢弃同轮已成功返回的实时状态；连续失败会指数退避，最长 60 秒。
- 拓扑管理改为只选入口、线路机和落地机；添加和修改立即保存，并按落地机地址自动创建或复用探测任务。
- 第 2 段会根据线路机已有的健康任务自动选择 ICMP 或 TCP；当前方式持续无响应时按常见 TCP 端口逐级回退。
- 关闭拓扑管理器后仍会定期检查第 2 段；已登录管理员的失效绑定会自动切换到同一落地机已验证可用的任务、端口或下一档探测方式。
- 新版 Metric Store 不可用时会回退旧版 Ping records，并严格按探测来源区分样本，避免其他线路机的成功数据掩盖当前线路故障。
- `0` 和其他亚毫秒成功样本统一显示为 `<1ms`；零丢包的亚毫秒波动保持绿色，100% 丢包和无响应仍显示为严重异常。
- 节点卡状态色使用贴合卡片最左边的真实边框，标题、状态点和内容保持一致的左侧间距。
...
入口是线路图上的标签，例如北京电信、北京联通。如果线路机上已经有同名三网 Ping 任务，第 1 段会自动绑定；没有也不影响添加。线路机到落地机的第 2 段任务由 Transit 按节点地址自动复用或创建，命名为 `Transit-线路机-to-落地机`，并且只分配给所选线路机。
...
- 第 1 段优先绑定线路机上已有的入口探测任务；没有对应任务时这一段先留空，不会伪造运营商数据。
...
入口只是线路图标签。只有线路机上已经存在对应的三网 Ping 任务时，第 1 段才会自动绑定。没有这个任务也不影响添加，第 2 段仍会按落地机地址自动创建。
