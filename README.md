...
- 当前稳定版：[v1.0.29](https://github.com/yyy622hhh/komari-theme-transit/releases/tag/v1.0.29)
...
- Ping 任务改为完整名称匹配；更换探测来源或跟随的线路机时，会清除不兼容的旧任务并要求重新选择。
...
每条线路至少需要两个名称不同的节点。两节点线路只有第 1 段，选择落地机后才会形成第 2 段。管理器会阻止缺少实时任务来源、节点重复、跳过中间位置或节点数量不足的配置。
...
拓扑使用完整任务名称进行精确匹配。例如 `北京电信`、`北京-电信` 和 `北京电信-备用` 是不同任务，不会被合并计算。更换探测来源或者更换被指标跟随的线路机时，管理器会清空旧 Ping 任务，避免把旧节点的任务名称误用到新节点。
...
如果刚刚更换了探测来源或线路机，任务留空是为了防止继续使用旧节点的任务。请为新节点重新选择或输入 Ping 任务。

### 拓扑一直显示备用值或 0 ms

检查来源节点、完整任务名称和目标是否对应正确链路。任务名称匹配成功但尚无样本时，等待至少一个采样周期。数据已过期或刷新失败时，页面会明确显示降级状态并使用备用基线。
