...
- 当前稳定版：[v1.1.0](https://github.com/yyy622hhh/komari-theme-transit/releases/tag/v1.1.0)
...
节点详情页和首页节点卡的标签行都会显示三条线路徽章，按档次着色（精品/优质/普通），鼠标悬停可以看到逐跳 ASN、判定依据和采集时间。没有采集数据时整块不渲染，主题里没有任何开关。

卡片上用紧凑形态：去掉「电信/联通/移动」前缀，改用一个运营商色点表示，配色与三网质量那一行一致。开启运维仪表盘后使用的卡片在 mini 密度下会隐藏整个标签行（既有的密度设计），回程徽章跟着一并不显示。
...
- **写回只动 `transit-route:` 那一条标签**，运营者自己的标签原样保留。
