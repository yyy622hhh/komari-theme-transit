...
- 当前稳定版：[v1.0.32](https://github.com/yyy622hhh/komari-theme-transit/releases/tag/v1.0.32)
...
实时链路指标来自 Komari Ping 任务。入口探测任务需要预先存在并分配给所选线路机；线路机到落地机的第 2 段任务可由 Transit 自动复用或创建。常见入口任务包括：

- 北京电信、北京联通、北京移动
- 上海电信、上海联通、上海移动
- 广东电信、广东联通、广东移动

任务名称可以自定义，但入口任务必须选择或填写准确名称。自动创建的第 2 段任务使用 `Transit-线路机-to-落地机` 命名，并只分配给所选线路机。
...
拓扑使用完整任务名称进行精确匹配。例如 `北京电信`、`北京-电信` 和 `北京电信-备用` 是不同任务，不会被合并计算。更换线路机或落地机时，管理器会按来源 UUID 与落地 IP 自动重新匹配；没有任务时会生成待创建计划，并在保存时创建，避免把旧节点的任务误用到新线路。
...
登录后打开拓扑管理器，先选**线路机**，再选可选的**落地机**，点击“快速生成”：

- 入口名称按线路机上的三网 Ping 任务自动填写；没有预设任务时使用“自定义入口”。
- 第 1 段绑定线路机正在执行的入口探测任务。
- 第 2 段按线路机 UUID 与落地机 IP 自动复用已有任务；不存在时会在保存时创建并绑定，不按名称或城市猜测。
- 不选落地机则生成“入口 → 线路机”两节点线路，空选项不会被节点刷新冲掉。
- 打开管理器时默认落地机避开已有线路；生成后自动切到下一台未使用的落地机。
- 相同线路机和落地机不会重复生成。
- 实时数据由线路机发出 Ping，图画方向不等于入口网络正向打过来。

生成结果是草稿，确认后点“保存并应用”才会写入 Komari。
