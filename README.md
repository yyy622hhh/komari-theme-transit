...
| 拓扑管理 | 选入口和线路机（落地机可选）后添加或修改，立即保存并自动创建或复用 ICMP/TCP 探测任务 |
...
登录后点击“线路状态”右上角的“管理”，选好入口和线路机即可添加线路（落地机可选）。添加会立即保存到 Komari 托管主题设置，并同步到所有设备。
...
配置一条线路需要两到三件事：入口、线路机是必选，落地机可以不选。
...
| 字段   | 必选 | 说明                                              |
| ------ | ---- | ------------------------------------------------- |
| 入口   | 是   | 北京 / 上海 / 广州的电信、联通或移动              |
| 线路机 | 是   | 承担中转或入口探测的服务器                        |
| 落地机 | 否   | 最终出口服务器；不选就只保存「入口 → 线路机」一段 |

登录后打开拓扑管理器，选好后点“添加线路”。落地机的下拉框默认是“不选（仅入口 → 线路机）”，保持不选会得到一条两节点线路，只创建或复用入口探测任务，不会有第 2 段。已有线路直接改下拉框即可，都会立即保存：
...
- 已添加的线路同样只显示入口、线路机和落地机；改这几项、调整顺序或删除都会立即保存。
  ...
  "routes": [
    {
      ...
      "metrics": [
        { "live": true, "source": "Relay-JP", "task": "北京电信" },
        { "live": true, "source": "Relay-JP", "task": "Relay-JP-to-Exit-US", "fallbackLatency": 84, "fallbackLoss": 0 }
      ]
    }
  ]
}
...
保存后等至少一个采样周期。第 2 段新建任务需要线路机先打到落地机 IP；主题会兼容新版 Metric Store 和旧版 Ping records，并在任务持续无响应时自动尝试其他可用探测方式。数据延迟或过期时页面会明确标出：超过 10 分钟没有新样本会显示上次更新时间，超过 30 分钟才回退到备用基线。`<1ms` 表示 Komari 返回了有效的亚毫秒样本（包括量化为 `0` 的成功样本），不是无数据；零丢包时应显示为正常绿色。
