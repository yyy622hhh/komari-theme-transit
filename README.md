...
- 当前稳定版：[v1.4.4](https://github.com/yyy622hhh/komari-theme-transit/releases/tag/v1.4.4)
...
- [快速开始](#快速开始) · [拓扑管理器](#拓扑管理器) · [Ping 与丢包口径](#ping-与丢包口径) · [三网回程线路](#三网回程线路)
...
- 节点卡三网区域聚焦 `Ping + 丢包率`。ICMP 才称为丢包率，TCP/HTTP 任务只称连接失败率，避免把端口探测误解成真实线路丢包。
- 拓扑节点间链路固定使用 ICMP；新任务先采集 30 个样本再参与告警和评分，避免 2～3 个初始失败被放大成 50%～100% 的稳定结论。
...
| 拓扑管理 | 内置/自定义入口、线路机、可选跳板与落地机；入口支持兼容回退，节点间链路固定 ICMP    |
...
- 第 1 段优先绑定线路机上已有的入口探测任务；站里已经有同名任务、只是没把这台线路机算进去时，会把线路机加进这个任务的节点列表而不是重复建一个；都没有才会自动新建并绑定。ICMP 不通时，内置入口可以验证并切换到同运营商同城市公共 DNS 的 TCP/53 候选。TCP 数值会明确写成“连接失败率”，不会伪装成丢包率。这测的是「线路机主动探测该地址」，方向和「该运营商用户访问线路机」相反。
- 后续节点间各段按来源节点 UUID 和目标节点 IP 自动复用或创建独立 ICMP 任务，只展示 Ping 与 ICMP 丢包率；旧版遗留的 TCP 绑定会安全迁回 ICMP，而不会把端口连接失败说成线路丢包。
...
1. 检查当前探测是否仍有新样本。节点间各段固定使用 ICMP，不再降级到 TCP；第 1 段使用入口目标，内置预设可在 ICMP 不通时验证 TCP/53（同运营商同城市的公共 DNS 解析器），自定义入口才使用 ICMP → TCP 443 → TCP 80 → TCP 22；
...
- Ping 数据的新鲜度按**每个任务的真实采样时间**判断，不是按「上次请求成功」——接口重复返回同一批旧记录不会被当成刷新成功。分两级：超过 10 分钟没有新样本，保留最后一次数据并在界面标出更新时间；超过 30 分钟才判定过期，当前指标改用备用基线，过期历史不会继续显示为实时数据。
...
节点间拓扑从 v1.4.3 起固定使用 ICMP，所以这部分正常情况下会统一显示 Ping 和 ICMP 丢包率。入口段为了兼容拒绝 ICMP 的公共目标，仍可能使用 TCP/53；遇到这种任务时保留“连接失败率”才是准确口径。
...
管理员可在“监测目标健康”中查看当前任务、分配节点、近一小时成功率和同步异常，并用临时 canary 任务验证候选目标。验证与迁移都需要明确确认；升级主题不会自动替换现有 Ping 目标。完整规则见[监测目标健康与安全迁移](docs/MonitoringTargetHealth.md)。
...
然后在每台节点下载**同一 GitHub Release** 的两个脚本并安装。推荐使用 HTTPS 面板设置向导生成命令。下面的 `v1.4.4` 应替换为实际已发布且与插件一致的标签，不要长期跟随会变化的 `main`。下载在独占私有目录中进行，任一失败即中止，不会执行当前目录的同名旧文件；安装器交互式读取 Agent token，输入不会进入 shell 历史：

```bash
(
  set -eu
  umask 077
  transit_release=v1.4.4
...
- 新结果保存在 Transit 主题数据中，不再写入 Komari 的用户标签。浏览器清理旧 `transit-route:` 时会重新读取节点最新标签，检测期间新增的运营者标签会保留。
...
- **新结果不写入节点标签**。主题会保存到 `pandaOpsRouteProbeResults`；如果节点仍有旧 `transit-route:`，先确认主题数据已保存，再重新读取最新节点只删除该保留条目，其他标签原样保留。
