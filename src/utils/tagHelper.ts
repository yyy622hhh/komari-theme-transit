...
function getExpiryDiffMs(expiredAt: string | number | null | undefined, now = Date.now()): number | null {
  if (!expiredAt)
    return null
 ...
 * 解析标签字符串为标签列表
 *
 * 带 `transit-route:` 保留前缀的条目会被滤掉：那是旧版本留下的回程判定数据，
 * 由迁移逻辑保存到主题设置后清理；迁移前仍应单独解析成线路徽章，不能再当成
 * 普通彩色标签展示一遍。
 *
