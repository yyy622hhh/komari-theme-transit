...
- `node-control.service.ts` — maintenance/silence control persistence for Transit node cards.
- `version.service.ts` — public Komari version lookup used by the footer.

## Focused source modules

- `stores/app.types.ts` — app store 的公开类型契约。
- `stores/app.settings.ts` — 主题预设、别名和托管设置解析纯函数；`stores/app.ts` 只负责响应式装配。
- `utils/metricRange.ts` — 负载图和延迟图共用的时间范围生成、校验与格式化纯函数。
- `utils/pingStats.ts` — Ping 历史分桶、百分位、丢包、可用性与 Metric Store 聚合纯计算。
- `utils/regionData.ts` — 地区旗帜、代码和别名数据；`regionHelper.ts` 只保留查询函数。
- `utils/homeQuickControls.ts` — 首页快捷筛选/排序纯函数。
- `utils/loadChartTheme.ts` — 详情负载图主题、坐标轴和 tooltip 纯配置。
...
These milestone layers are in place and remain the contract for new work:
...
- M3 gates private tools, sensitive metadata, snapshot export, disk prediction and node-card panel writes through verified auth; ordinary node and Ping history metrics stay public.
