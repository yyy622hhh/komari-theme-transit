...
- 状态：in-progress，准备将项目以 `Transit 1.0.0` 发布到 `yyy622hhh/komari-theme-transit`
- 目标：完成对外品牌重命名、项目 README、产品截图、可复现的 komari-web 后台补丁、二开来源与 MIT 致谢，并直接推送到目标仓库 `main`。
- 范围：M6 文档/发布与必要的品牌一致性修改；保留现有 PandaOps 内部类型、存储键和 CSS 类名，避免破坏已部署用户的设置兼容性。
- 发布边界：目标仓库当前只有一个 README 占位提交；发布时合并并保留该提交历史，不强推，不修改现有 origin/upstream。
...
- 设计：保持首页高密度拓扑结构，不新增独立大卡片；可靠性与排名只在当前线路摘要和详情里出现，使用数字、短理由和现有语义色，不使用比较进度条。
- 数据边界：只复用 Komari 公开 Ping 历史和统计接口；智能基线基于线路自身历史中位数/P95，不跨线路套固定延迟阈值；静态备用值不冒充真实 SLO。
- 实现：扩展共享 Ping 统计，新增 P50、P95、样本量和可用率；每段后台维护 24h/7d 窗口，完整线路按最弱分段汇总可用率，并以当前整线延迟相对自身 24h P50/P95 判断基线偏离；同一落地方向按当前健康分、24h 可用率和相对延迟生成唯一推荐与短理由。
- 边界处理：任一分段只有静态备用值或缺少历史时，整线 SLO 显示数据覆盖度，不将备用值计为真实可用率；排名仍可参考当前健康状态，但会明确历史仍在积累。
- 验证：`bun run lint`、`bun run type-check`、`bun run build-only`、`git diff --check` 通过；完整 Playwright 深浅色、桌面、移动端与交互回归 29/29 通过，新增同方向唯一推荐、智能基线、24h/7d/P95、推荐理由、静态分段覆盖度和无溢出断言。
- 发布：提交 `9d51aba`，构建包 `komari-theme-PandaOps-build-9d51aba.zip`，本机与服务器 SHA-256 均为 `cd05a34ff9f9ef2b156273a2a3b090f834ac3f4ba4ba3a8d47b5a809eb94047c`。旧活动主题完整备份到 `/var/lib/komari/backups/PandaOps-panda19-before-panda20-20260814`；首次切换因重启后就绪探测过早触发自动回滚，确认旧版恢复健康后改用带重试的就绪检查完成第二次原子切换。Komari 服务 active，本机与公网首页 200，线上入口为 `index-onIiWy9-.js`，首页资源为 `HomeView-Ba3XOHVs.js`，PandaOps 资源为 `PandaOpsDashboard-IHux5NWc.js`；线上代码已确认包含智能基线、24h/7d 可用率、推荐依据和历史数据积累提示。
...
- 里程碑：M2 性能 + M3 安全 + M4 UI/UX + M5 告警状态收口；保留现有信息架构、拓扑配置格式、公开/私有权限边界和深浅色视觉语言。
- 结果：集中告警注册表与连续采样确认、Ping 新鲜度、采样浮窗单例监听、节点卡片尺寸、移动端资产/异常摘要均已完成；`lint`、`type-check`、`build` 和 27 项 Playwright 回归通过。主题以可回滚目录原子部署，Nginx 已启用 gzip、静态资源一年缓存、动态页面不缓存、Permissions-Policy，并避免大脚本临时落盘和客户端令牌写入错误日志。
- 不做：不新增公网 IP 检测，不改变节点、Ping 任务或拓扑数据，不启用会中断现有后台/自定义背景的强制 CSP。
